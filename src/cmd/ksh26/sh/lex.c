		...
		switch(n)
		{
				...
				if(state[n] == S_OP || n == '#')
				{
					if(n == c)
					{
						if(c == LPAREN)
						{
							...
							if(lp->lexd.dolparen && !lp->lexd.dolparen_arithexp && (fcpeek(-2) == '=' || lp->lexd.dolparen_eqparen))
								return lp->token = c;
			...
			case S_BRACE:
			{
				...
				if(lp->lex.reservok && state[n] == S_BREAK && isfirst)
					break;
#if SHOPT_BRACEPAT
				if(sh_isoption(SH_BRACEEXPAND) && c == LBRACE && !assignment && state[n] != S_BREAK && !lp->lex.incase && !lp->lex.intest && !lp->lex.skipword)
				{
					wordflags |= ARG_EXP;
				}
#endif
	...
	if(!lp->lex.skipword)
	{
		...
		if(!(wordflags & ARG_QUOTED) && (lp->lex.reservok || lp->aliasok))
		{
			...
			if(!lp->lex.incase && !assignment && fcpeek(0) != LPAREN &&
			   (np = nv_search(state, sh.alias_tree, 0)) && !nv_isattr(np, NV_NOEXPAND) && (lp->aliasok != 2 || nv_isattr(np, BLT_DCL)) && (!sh_isstate(SH_NOALIAS) || nv_isattr(np, NV_NOFREE)) && (state = nv_getval(np)))
			{
