...
void nv_setlist(struct argnod *arg, int flags, Namval_t *typ)
{
	...
	struct sh_type shtp;
	Dt_t *vartree, *save_vartree = nullptr;
#if SHOPT_NAMESPACE
		...
		else
		{
				...
				if(tp->tre.tretyp != TLST && tp->com.comarg.ap && !tp->com.comset && ((array & NV_IARRAY) || !((mp = tp->com.comnamp) && nv_isattr(mp, BLT_DCL))))
				{
					...
					if(traceon || trap)
					{
						...
						if(arg->argflag & ARG_APPEND)
							n = '+';
						if(trap)
							sh_debug(trap, name, nullptr, argv, (arg->argflag & ARG_APPEND) | ARG_ASSIGN);
						if(traceon)
						{
							sh_trace(nullptr, 0);
			...
			skip:
				if(sub > 0)
				{
					...
					nv_putsub(np, nullptr, ARRAY_ADD | ARRAY_FILL | sub);
				}
				else if(prefix)
...
static char *copystack(const char *prefix, const char *name, const char *sub)
{
	...
	if(last)
	{
		...
		if(sh_checkid(stkptr(sh.stk, last), nullptr))
	...
	while(1)
	{
		switch(c = *(unsigned char *)(sp = cp))
		{
			case '[':
				...
				cp = nv_endsubscript(nullptr, sp, 0);
				if(sp == name || sp[-1] == '.')
					c = *(sp = cp);
				goto skip;
			case '.':
				...
				if(!copy && !(flags & NV_NOREF))
				{
					...
					copy = cp - name;
					dp->nofree |= 1;
					name = copystack(nullptr, name, nullptr);
					cp = (char *)name + copy;
					sp = (char *)name + c;
					c = '.';
				}
			...
			case 0:
				...
				if(top)
				{
					...
					{
						Dt_t *dp = dtview(sh.var_tree, nullptr);
					...
					if(mp = nv_search(name, sh.var_tree, 0))
					{
...
#endif /* SHOPT_NAMESPACE */
						{
							...
							if((flags & NV_NOSCOPE) && *cp != '.')
							{
								...
								{
									nv_delete(mp, nullptr, NV_NOFREE);
									mp = 0;
								}
							}
							np = mp;
 ...
 * if <root> is nullptr, only the cache is cleared
...
Namval_t *nv_open(const char *name, Dt_t *root, int flags)
{
	...
	if(root == sh.fun_tree)
	{
		...
		if(strchr(name, '.'))
		{
			name = cp = copystack(0, name, nullptr);
			fname = strrchr(cp, '.');
			*fname = 0;
			fun.nofree |= 1;
			flags &= ~NV_IDENT;
	...
	{
		name = cp = copystack(sh.prefix, name, nullptr);
		fun.nofree |= 1;
	}
	c = *(unsigned char *)cp;
		...
		else
		{
			...
			if(flags & NV_MOVE)
			{
				if(ap = nv_arrayptr(np))
				{
					...
					{
						ap->nelem |= ARRAY_TREE;
						nv_putsub(np, nullptr, ARRAY_ADD | nv_aindex(np));
						np = nv_opensub(np);
						ap->nelem &= ~ARRAY_TREE;
						ap->nelem -= 1;
					}
...
void nv_putval(Namval_t *np, const char *sp, int flags)
{
	...
	else
	{
		...
		{
			char *cp = nullptr;         /* pointer to new string */
			...
			if(nv_isattr(np, NV_BINARY))
			{
				...
				*vpp = cp;
				if(size <= oldsize)
					return;
				dot = base64decode(sp, dot, nullptr, cp + oldsize, size - oldsize, nullptr);
				dot += oldsize;
...
char **sh_envgen(void)
{
	...
	namec = nv_scan(sh.var_tree, nullscan, nullptr, NV_EXPORT, NV_EXPORT);
	namec += sh.save_env_n;
...
void nv_unset(Namval_t *np, int flags)
{
			...
			if(cp)
			{
				...
				*cp++ = '.';
				if(npv && npv != sh.namespace)
					nv_setdisc(npv, cp, nullptr, (Namfun_t *)npv);
			}
	...
	if(np->nvfun && !nv_isref(np))
	{
		...
		{
			nv_local = 1;
			nv_putv(np, nullptr, flags, np->nvfun);
			nv_local = 0;
			return;
		}
		/* called from disc, assign the actual value */
...
char *nv_getval(Namval_t *np)
{
	...
	if(vp && nv_isattr(np, NV_BINARY) && !nv_isattr(np, NV_RAW))
	{
		...
		base64encode(vp, size, nullptr, cp = getbuf(insize), insize, (void **)&ep);
		*ep = 0;
		return cp;
	}
	...
	while(c = mbchar(cp))
	{
		if(c == '[')
		{
			if(*cp == ']')
				...
				cp = nv_endsubscript(nullptr, (ep = cp) - 1, 0);
		}
		else if(c == '.')
		{
			if(*cp == '[')
			{
				cp = nv_endsubscript(nullptr, ep = cp, 0);
...
int nv_rename(Namval_t *np, int flags)
{
	...
	if((nv_arrayptr(nr) && !arraynr) || nv_isvtree(nr))
	{
		...
		if(flags & NV_MOVE)
		{
			...
			nv_delete(nr, nullptr, NV_NOFREE);
		}
	}
	else
	{
...
void nv_setref(Namval_t *np, Dt_t *hp, int flags)
{
	...
	if(nr == np)
	{
		...
		if(nv_isarray(nq))
			nv_putsub(nq, nullptr, ARRAY_UNDEF);
	}
#if SHOPT_FIXEDARRAY
...
#if SHOPT_FIXEDARRAY
		if(ap && ap->fixed)
			nrp->curi = ARRAY_FIXED | nv_arrfixed(nq, nullptr, 1, &nrp->dim);
		else
#endif /* SHOPT_FIXEDARRAY */
...
#if SHOPT_FIXEDARRAY
	if(ap && ap->fixed)
		nv_arrfixed(np, sh.strbuf, 1, nullptr);
#endif /* SHOPT_FIXEDARRAY */
	return sfstruse(sh.strbuf);
}
