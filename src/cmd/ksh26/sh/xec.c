...
int sh_debug(const char *trap, const char *name, const char *subscript, char *const argv[], int flags)
{
	...
	/* three-layer nesting: stk outermost, lite polarity middle, no inner checkpt (allocator boundaries) */
...
int sh_eval(sh_stream_t *iop, int mode)
{
	...
	sh.binscript = 0;
	sh.comsub = 0;
	/* computation-only: eval/source stream execution (polarity boundary) */
...
{
	if(type & (FAMP | FPOU) || !(execflg && sh.fn_depth == 0 || execflg2) || sh.st.trapdontexec || sh.subshell || ((struct checkpt *)sh.jmplist)->mode == SH_JMPEVAL || sh_isstate(SH_XARG) || (pipejob && (sh_isstate(SH_MONITOR) || sh_isoption(SH_PIPEFAIL) || sh_isstate(SH_TIMING))))
	{
		return 0;
	}
...
int sh_exec(const Shnode_t *t, int flags)
{
	...
	else if(type == TCOM && !sh.dont_optimize_builtins) /* no COMSCAN bit in 'type' => no expansions */
	{
		Namval_t *np;
		Shbltin_f fp;
		if((np = (Namval_t *)t->com.comnamp) && (fp = funptr(np)) && (fp == b_true || (fp == b_false && !sh_isoption(SH_ERREXIT) && !sh.st.trap[SH_ERRTRAP]) || fp == b_break && t->com.comarg.dp->dolnum == 1) /* for break/continue: 1 arg (command name) */
		   && !t->com.comset                                                                                                                                                                                    /* no variable assignments list */
		   && !t->com.comio                                                                                                                                                                                     /* no I/O redirections */
		   && !sh_isoption(SH_XTRACE) && !sh.st.trap[SH_DEBUGTRAP])
		{
	...
	/* Normal command execution */
	{
		...
		int argn;
		int skipexitset = 0; /* ⊕ gating: prevents exitset() commit to $? (error duality) */
		...
		/* ⊕→⅋ suppression: strip SH_ERREXIT in conditional contexts (error duality) */
		if(!(flags & sh_state(SH_ERREXIT)))
			sh_offstate(SH_ERREXIT);
		/* Polarity classification: see sh_node_polarity[] in shnodes.h (polarity classification). */
		switch(type & COMMSK)
		{
			/*
		     * Simple command
			...
			case TCOM: /* mixed: assignments (value) + execution (computation) */
			{
				...
				if(sh.envlist = argp = t->com.comset)
				{
					if(argn == 0 || (np && (nv_isattr(np, BLT_DCL) || (!command && nv_isattr(np, BLT_SPC)))))
					{
						...
						else
						{
							...
							/* computation-only: assignment error recovery (polarity boundary) */
				...
				if(io || argn)
				{
					...
					if(np && is_abuiltin(np))
					{
						...
						/* computation-only: builtin execution (polarity boundary) */
					...
					if(!command && np && nv_isattr(np, NV_FUNCTION))
					{
						...
						{
							indx = sh.topfd;
							/* computation-only: I/O redirection for builtin (polarity boundary) */
			 ...
			 */
				{
					...
					if(no_fork)
						sh_sigreset(2);
					/* computation-only: child process setup (polarity boundary) */
			...
			case TSETIO: /* mixed: redirection setup (value) + subtree execution */
			{
				...
				/* computation-only: I/O redirection for command (polarity boundary) */
				sh_pushcontext(buffp, SH_JMPIO);
				if(type & FPIN)
				{
			...
			case TPAR: /* computation: subshell boundary */
				...
				if(!sh.subshell && !sh.st.trapdontexec && (flags & sh_state(SH_NOFORK)))
				{
					...
					sh.realsubshell++;
					sh_sigreset(0);
					/* computation-only: virtual subshell/group execution (polarity boundary) */
					sh_pushcontext(buffp, SH_JMPEXIT);
					jmpval = sigsetjmp(buffp->buff, 0);
					if(jmpval == 0)
			...
			case TFOR: /* mixed: list expansion (value) + loop body (computation) */
			{
				...
				/* computation-only: for-loop optimizer checkpoint (polarity boundary) */
			...
			case TWH: /* mixed: condition (value) + loop body (computation) */
			{
				...
				/* computation-only: while-loop optimizer checkpoint (polarity boundary) */
				sh_pushcontext(buffp, jmpval);
				jmpval = sigsetjmp(buffp->buff, 0);
				if(jmpval)
					goto endwhile;
				...
				always_true = (tt->tre.tretyp == TCOM                                                                                                                                                       /* one simple command (no COMSCAN = no expansions) */
				               && !sh.dont_optimize_builtins && (np = (Namval_t *)tt->com.comnamp) && (fp = funptr(np)) && (type == TWH && fp == b_true || type == TUN && fp == b_false) && !tt->com.comset /* no variable assignments list */
			...
			case TFUN: /* mixed: symbol table (value) + namespace body (computation) */
			{
				...
				if(t->tre.tretyp == TNSPACE)
				{
					...
					/* computation-only: namespace body execution (polarity boundary) */
					sh_pushcontext(chkp, SH_JMPCMD);
					jmpval = sigsetjmp(chkp->buff, 0);
					if(!jmpval)
				...
				if(cp || sh.prefix)
				{
					/* within-value: isolate prefix for discipline nv_open (prefix isolation) */
					int offset = stktell(sh.stk);
					if(sh.prefix)
					{
		...
		/* ⊕→⅋ bridge: nonzero sh.exitval + SH_ERREXIT → sh_chktrap (error duality) */
		 ...
		 * intentional: loop body allocations persist for the optimizer (allocator boundaries) */
...
int sh_funscope([[maybe_unused]] int argn, char *argv[], int (*fun)(void *), void *arg, int execflg)
{
	...
	*prevscope = sh.st;
	...
	/* scope boundary: full function scope (polarity boundary) */
	...
	sh_scope_set((Dt_t *)prevscope->own_tree);
...
int sh_fun(Namval_t *np, Namval_t *nq, char *argv[])
{
	...
	Fcin_t save;
	fcsave(&save);
	/* three-layer nesting: stk outermost, polarity middle, checkpt innermost (allocator boundaries) */
	...
	/* polarity boundary: sh_polarity_enter above (polarity boundary) */
...
static pid_t sh_ntfork(const Shnode_t *t, char *argv[], int *jobid, int topfd)
{
	...
	Pathcomp_t *pp;
	/* computation-only: process spawn attempt (polarity boundary) */
	sh_pushcontext(buffp, SH_JMPCMD);
