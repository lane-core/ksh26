...
static Namval_t *scope(Namval_t *np, struct lval *lvalue, int assign)
{
	...
	if(cp >= lvalue->expr && cp < lvalue->expr + lvalue->elen)
	{
		...
		int c = cp[flag];
		cp[flag] = 0;
		if((!(np = nv_open(cp, sh.var_tree, assign | NV_VARNAME | NV_NOADD | NV_NOFAIL)) || nv_isnull(np)) && sh_macfun(cp, offset = stktell(sh.stk)))
		{
		...
		case LOOKUP:
		{
			...
			if(isaletter(c))
			{
				...
				if(c == '(')
				{
					...
					c = **ptr;
					lvalue->fun = 0;
					stkprintf(sh.stk, ".sh.math.%.*s%c", fsize, *ptr, 0);
					stkseek(sh.stk, off);
