...
char *sh_mactry(char *string)
{
	if(string)
	{
		...
		/* computation-only: error recovery for macro expansion (polarity boundary) */
			...
			do
			{
				...
				do
				{
					if(LEN == 1)
						...
						stkwrite(stkp, fcseek(0) - LEN, LEN);
...
retry2:
	if(v && (!nulflg || *v) && c != '+')
	{
		...
		while(1)
		{
			...
			{
				sh_stream_t *sp = mp->sp;
				if(mp->pattern && mp->quote && mp->ifs)
					sfputc(sp, '\\');
				if(mode == '@' || !mp->ifsp)
					sfputc(sp, ' ');
				else if(mp->ifs)
				{
					if(!mbwide() || mp->ifs < 128)
						sfputc(sp, mp->ifs);
					else
					{
						if(!ofs_size)
			...
			else
			{
				...
				else if(mp->ifs)
				{
					...
					else
					{
						...
						stkwrite(stkp, mp->ifsp, ofs_size);
					}
				}
			}
		}
...
static void comsubst(Mac_t *mp, Shnode_t *t, int type)
{
	...
	if(t)
	{
		...
		if(t->tre.tretyp == 0 && !t->com.comarg.dp && !t->com.comset)
		{
			...
			struct checkpt buff;
			struct ionod *ip = 0;
			/* computation-only: I/O for $(<file) (polarity boundary) */
...
static void tilde_expand2(int offset)
{
	...
	if(cp)
	{
		...
		if(!(cp[0] == '/' && !cp[1] && fcpeek(0) == '/'))
			stkputs(sh.stk, cp, -1); /* for ~ == /, avoid ~/foo -> //foo */
	}
	else
