...
static Shnode_t *funct(Lex_t *lexp)
{
	...
	if((flag && lexp->token != LBRACE) || lexp->token == EOFSYM)
		sh_syntax(lexp, 0);
	/* computation-only: parse error recovery for function body (polarity boundary) */
...
static Shnode_t *item(Lex_t *lexp, int flag)
{
	...
	switch(tok)
	{
		...
		case IFSYM:
		{
			...
			tok = lexp->token;
			t->if_.eltre = (tok == ELSESYM ? sh_cmd(lexp, FISYM, SH_NL | SH_SEMI) : (tok == ELIFSYM ? (lexp->token = IFSYM, tt = item(lexp, SH_NOIO)) : 0));
			if(tok == ELIFSYM)
			{
