...
static int read_tree(Namval_t *np, Sfio_t *iop, int n, [[maybe_unused]] Namfun_t *dp)
{
	...
	sp = sfopen(nullptr, sfstruse(sh.strbuf), "s");
	sfstack(iop, sp);
...
static char *nextdot(const char *str)
{
	...
	for(cp = (char *)str; c = *cp; cp++)
	{
		if(c == '[')
		{
			cp = nv_endsubscript(nullptr, (char *)cp, 0);
			return *cp == '.' ? cp : 0;
		}
		if(c == '.')
			return cp;
		...
		if(np && ((nfp = nextdisc(np)) || nv_istable(np)))
		{
			...
			*save = *dp;
			...
			if(nfp)
			{
				...
				dp->fun = nfp;
				dp->hp = (*dp->nextnode)(np, nullptr, nfp);
			}
			else
				dp->nextnode = 0;
		}
		...
		while(np = dp->hp)
		{
			...
			if(dp->nextnode && !dp->hp && (nq = (Namval_t *)dp->table))
			{
				...
				if(ap && (ap->nelem & ARRAY_SCAN) && nv_nextsub(nq))
					dp->hp = (*dp->nextnode)(np, nullptr, dp->fun);
			}
...
static void outval(char *name, const char *vname, struct Walk *wp)
{
	...
	if(special || (nv_isarray(np) && nv_arrayptr(np)))
	{
		...
		if(array_elem(nv_arrayptr(np)) == 0)
			...
			nq = nv_putsub(np, nullptr, ARRAY_SCAN | (wp->out ? ARRAY_NOCHILD : 0));
	}
	if(!wp->out)
	{
	...
	if(!special)
	{
		if(*name != '.')
		{
			...
			if((ap = nv_arrayptr(np)) && ap->fixed)
			{
				...
				nv_arrfixed(np, wp->out, 0, nullptr);
				sfputc(wp->out, ';');
			}
#endif
		}
	...
	for(; arg = *argv; argv++)
	{
		...
		if(n == 0 || strncmp(arg, prefix - n, m + n) == 0)
		{
			...
			else if(outfile && *cp == '[' && cp[-1] != '.')
			{
				...
				if(*nv_endsubscript(nullptr, cp, 0) == '[')
					continue;
				if(wp->indent > 0)
					sfnputc(outfile, '\t', wp->indent);
				if(cp[-1] == '.')
...
static char *walk_tree(Namval_t *np, Namval_t *xp, int flags)
{
	...
	*argv = 0;
	...
	if(flags & 1)
		...
		outfile = out = sfnew(nullptr, nullptr, -1, -1, SFIO_WRITE | SFIO_STRING);
...
char *nv_getvtree(Namval_t *np, Namfun_t *fp)
{
	...
	if(nv_isattr(np, NV_BINARY) && !nv_isattr(np, NV_RAW))
		return nv_getv(np, fp);
	if(nv_isattr(np, NV_ARRAY) && !nv_type(np) && nv_arraychild(np, nullptr, 0) == np)
		return nv_getv(np, fp);
	...
	if(dsize && (flags & NV_EXPORT))
		return "()";
	return walk_tree(np, nullptr, flags);
}
...
static void put_tree(Namval_t *np, const char *val, int flags, Namfun_t *fp)
{
	...
	if(!nv_isattr(np, (NV_INTEGER | NV_BINARY)))
	{
		...
		if(mp && nv_isvtree(mp))
		{
			...
			if(!(flags & NV_APPEND))
				walk_tree(np, nullptr, (flags & NV_NOSCOPE) | 1);
...
void nv_setvtree(Namval_t *np)
{
	...
	if(nv_hasdisc(np, &treedisc))
		return;
	nfp = sh_newof(nullptr, Namfun_t, 1, 0);
	nfp->disc = &treedisc;
