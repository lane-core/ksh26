...
int b_alias(int argc, char *argv[], Shbltin_t *context)
{
	...
	if(argv[1])
	{
		...
		*opt_info.option = 0;
		tdata.argnum = 0;
		tdata.aflag = *argv[1];
		while((n = optget(argv, *argv[0] == 'h' ? sh_opthash : sh_optalias)))
			switch(n)
			{
				case 'p':
					tdata.prefix = argv[0];
...
static int setall(char **argv, int flag, Dt_t *troot, struct tdata *tp)
{
	...
	if(argv[1])
	{
		...
		while(name = *++argv)
		{
			...
			if(np->nvflag & NV_RDONLY && !tp->pflag && (tp->aflag == '+' || flag & ~(NV_ASSIGN | NV_RDONLY | NV_EXPORT))) /* allow readonly/export on readonly vars */
			{
...
static int unall(int argc, char **argv, Dt_t *troot)
{
	...
	while(name = *argv++)
	{
		/* computation-only: error recovery for typeset/export/readonly (polarity boundary) */
