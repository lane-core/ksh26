...
static void refresh(Vi_t *vp, int mode)
{
	...
	v = cur_virt;
	if(v < vp->ocur_virt || vp->ocur_virt == INVALID || (v == vp->ocur_virt && (!is_print(virtual[v]) || !is_print(vp->o_v_char))))
	{
		opflag = 0;
		p = 0;
		v = 0;
	...
	if((p > last_phys || p >= first_w + w_size) && w > vp->last_wind && cur_virt == vp->ocur_virt)
	{
...
static void replace(Vi_t *vp, int c, int increment)
{
	...
	if(vp->ocur_virt == INVALID || !is_print(c) || !is_print(virtual[cur_virt]) || !is_print(vp->o_v_char)
#if SHOPT_MULTIBYTE
	   || !iswascii(c) || mbwidth(vp->o_v_char) > 1 || !iswascii(virtual[cur_virt])
#endif /* SHOPT_MULTIBYTE */
