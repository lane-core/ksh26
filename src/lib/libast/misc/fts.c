...
FTS *fts_open(char *const *pathnames, int flags, int (*comparf)(FTSENT *const *, FTSENT *const *))
{
	FTS *fts;
			 ...
			 */
				 ...
				 */
					...
					if(fts->cd <= 0)
					{
						...
						if(fts->cd < 0 || f != fts->curdir || !fts->dotdot || !SAME(f->fts_statp, fts->dotdot->fts_statp) || fts->pwd && fts->pwd->symlink || (fts->cd = chdir("..")) < 0
#ifdef verify
						   || stat(".", &sb) < 0 || !SAME(&sb, fts->dotdot->fts_statp)
#endif
						)
...
int fts_set(FTS *fts, FTSENT *f, int status)
{
...
int fts_notify(Notify_f notifyf, void *context)
{
	Notify_t *np;
	Notify_t *pp;
