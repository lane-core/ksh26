...
skip:
	...
	for(;;)
	{
		...
		if((!starstar && !gp->gl_starstar || (t1 = (*gp->gl_type)(gp, dirname, GLOB_STARSTAR)) == GLOB_DIR || t1 == GLOB_SYM && pat[0] == '*' && pat[1] == '\0') /* follow symlinks to dirs for non-globstar components */
		   && (dirf = (*gp->gl_diropen)(gp, dirname)))
		{
			if(!(gp->re_flags & REG_ICASE) && (gp->gl_flags & GLOB_DCASE) && ((*gp->gl_attr)(gp, dirname, 0) & GLOB_ICASE))
			{
				if(!prei)
				{
			...
			while((name = (*gp->gl_dirnext)(gp, dirf)) && !*gp->gl_intr)
			{
				 ...
				 */
				if(!(matchdir && (pat[0] == '.' && (!pat[1] || pat[1] == '.' && !pat[2]) || strchr(pat, '/'))) && name[0] == '.' && (!name[1] || name[1] == '.' && !name[2]) && !(gp->gl_flags & GLOB_FCOMPLETE))
					continue;
...
int _ast_glob(const char *pattern, int flags, int (*errfn)(const char *, int), glob_t *gp)
{
	...
	else
	{
		...
		*argv = 0;
		...
		{
			strsort(av, argv - av, ast.locale.collate);
			if(gp->gl_starstar > 1)
