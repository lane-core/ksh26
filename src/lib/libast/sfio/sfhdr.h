...
#define SFMVUNSET(f) (!((f)->bits & SFIO_MVSIZE) ? 0 : (((f)->bits &= ~SFIO_MVSIZE), ((f)->size /= SFIO_NMAP)))
...
#define _SFOPEN(f) ((f)->mode == SFIO_READ ? _SFOPENRD(f) : (f)->mode == SFIO_WRITE ? _SFOPENWR(f) \
	                                                                            : ((f)->endw = (f)->endr = (f)->data))
#define SFOPEN(f, l) (void)((l) ? 0 : ((f)->mode &= ~(SFIO_LOCK | SFIO_RC | SFIO_RV), _SFOPEN(f), 0))

/* check to see if the stream can be accessed */
#define SFFROZEN(f) (((f)->mode & (SFIO_PUSH | SFIO_LOCK | SFIO_PEEK)) ? 1 : !((f)->mode & SFIO_STDIO) ? 0                \
	                                                                 : _Sfstdsync                  ? (*_Sfstdsync)(f) \
	                                                                                               : (((f)->mode &= ~SFIO_STDIO), 0))
...
#define SFDISC(f, dc, iof)                                                        \
	{                                                                         \
		...
		if(!(dc))                                                         \
			...
			d = (f->bits & SFIO_DCDOWN) ? ((dc) = (dc)->disc) : (dc); \
		while(d && !(d->iof))                                             \
			d = d->disc;                                              \
		if(d) (dc) = d;                                                   \
	}
...
#define SFRPEEK(f, s, n) (_SFAVAIL(f, s, n) > 0 ? (n) : ((n) = SFFILBUF(f, -1), (s) = (f)->next, (n)))
#define SFWPEEK(f, s, n) (_SFAVAIL(f, s, n) > 0 ? (n) : ((n) = SFFLSBUF(f, -1), (s) = (f)->next, (n)))
