...
#define mbnchar(p, n) (mbwide() ? ((ast.mb.tmp_i = (*ast.mb.towc)(&ast.mb.tmp_w, (char *)(p), n)) > 0 ? ((p += ast.mb.tmp_i), ast.mb.tmp_w) : (p += ast.mb.sync + 1, ast.mb.tmp_i)) : (*(unsigned char *)(p++)))
...
#define NoP(x)             \
	do                 \
		(void)(x); \
	while(0) /* for silencing "unused parameter" warnings */
