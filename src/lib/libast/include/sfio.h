...
#define __sf_putc(f, c) (_SFIO_(f)->_next >= _SFIO_(f)->_endw ? _sfflsbuf(_SFIO_(f), (int)((unsigned char)(c))) : (int)(*_SFIO_(f)->_next++ = (unsigned char)(c)))
#define __sf_getc(f) (_SFIO_(f)->_next >= _SFIO_(f)->_endr ? _sffilbuf(_SFIO_(f), 0) : (int)(*_SFIO_(f)->_next++))
...
#define __sf_ulen(v) ((Sfulong_t)(v) < SFIO_U1 ? 1 : (Sfulong_t)(v) < SFIO_U2 ? 2 \
	                                         : (Sfulong_t)(v) < SFIO_U3   ? 3 \
	                                         : (Sfulong_t)(v) < SFIO_U4   ? 4 \
	                                                                      : 5)
...
#define sfstrseek(f, p, m)                                                                                                                               \
	((m) == SEEK_SET   ? (((p) < 0 || (p) > (f)->_size) ? (char *)0 : (char *)((f)->_next = (f)->_data + (p)))                                       \
	 : (m) == SEEK_CUR ? ((f)->_next += (p),                                                                                                         \
	                      (((f)->_next < (f)->_data || (f)->_next > (f)->_data + (f)->_size) ? ((f)->_next -= (p), (char *)0) : (char *)(f)->_next)) \
	 : (m) == SEEK_END ? (((p) > 0 || (f)->_size < -(p)) ? (char *)0 : (char *)((f)->_next = (f)->_data + (f)->_size + (p)))                         \
	                   : (char *)0)
