# probe: ast-stdio — sfio stdio compatibility layer (~385 lines)
# Tier 5 (complex). Detects FILENAME_MAX, FOPEN_MAX, TMP_MAX,
# P_tmpdir, L_ctermid, L_tmpnam, and emits the full stdio wrapper
# with FILE typedef, function remaps, and sfio integration macros.
#
# Lifted from monolith probe_ast_stdio with API translations:
...
probe_ast_stdio()
{
	...
	if [ "$opt_force" = 0 ] && [ -f "$_out" ] \
	   && [ "$_out" -nt "$LIBAST_SRC/features/stdio" ]; then
		return 0
	fi

	# The output{} block compiles and runs to discover FILENAME_MAX etc.
	_stdio_output=$(_mc_output <<'EOF'
#include <stdio.h>
#ifndef FILENAME_MAX
#ifndef NAME_MAX
...
EOF
	...
	# macro{} block for P_tmpdir, L_ctermid, L_tmpnam
	_stdio_macro=$(_mc_output <<'EOF'
#include <stdio.h>
int
main(void)
{
...
EOF
	...
	{
		echo "/* : : generated by configure.sh probe_ast_stdio : : */"
		echo "#ifndef _SFSTDIO_H"
		echo "#define _SFSTDIO_H	1"
		echo "#define _sys_types	1	/* #include <sys/types.h> ok */"
		# cat{} block #1 — FILE typedef setup
		cat <<'STDIOCAT1'
...
STDIOCAT1
		# output{} result
		[ -n "$_stdio_output" ] && printf '%s\n' "$_stdio_output"
		# macro{} result
		[ -n "$_stdio_macro" ] && printf '%s\n' "$_stdio_macro"
		# cat{} block #2 — function declarations
		cat <<'STDIOCAT2'
