# probe: ast-ccode — character code detection (ASCII vs EBCDIC)
# Tier 5. Runs a C program that detects the native character encoding
# by examining character constants.
#
# Lifted from monolith probe_ast_ccode with API translations:
...
probe_ast_ccode()
{
	...
	if [ "$opt_force" = 0 ] && [ -f "$_out" ] \
	   && [ "$_out" -nt "$LIBAST_SRC/features/ccode" ]; then
		return 0
	fi

	# The ccode output{} block is a pure printf program — no includes needed
	_cc_result=$(_mc_output <<'EOF'
#include <stdio.h>
int main(void)
{
	printf("\n");
...
EOF
	...
	{
		echo "/* : : generated by configure.sh probe_ast_ccode : : */"
		echo "#ifndef _def_ccode_ast"
		echo "#define _def_ccode_ast	1"
		echo "#define _sys_types	1	/* #include <sys/types.h> ok */"
		[ -n "$_cc_result" ] && printf '%s\n' "$_cc_result"
		echo "#endif"
