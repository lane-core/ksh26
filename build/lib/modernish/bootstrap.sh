#! /bin/sh
# Wrapper script to run bootstrap.sh with bundled modernish
...
MSH_SHELL=/etc/profiles/per-user/lane/bin/sh
. "$MSH_PREFIX/lib/modernish/adj/goodsh.sh" || exit
case $MSH_SHELL in
( */sh )	;;
( * )	echo bootstrap.sh: "warning: sh shell not usable; running on $MSH_SHELL" >&2 ;;
esac

# Prefix launch arguments.
set -- "$MSH_PREFIX/bin/modernish" "$MSH_PREFIX"/bin/bootstrap.sh "$@"
